...
export default async function OfficePage({ searchParams }: PageProps) {
  ...
  const office = await prisma.show.findFirst({ where: { name: { contains: 'OFFICE' } } })
  if (!office) return notFound()

  const [episodes, speakers] = await Promise.all([
    prisma.episode.findMany({
      where: { showId: office.id },
  ...
  return (
    <div className="min-h-screen" style={{ background: 'linear-gradient(180deg, #87CEEB 0%, #B0E0FF 100%)' }}>
      <SiteHeader
        ...
        subtitle="Search The Office quotes"
      />
